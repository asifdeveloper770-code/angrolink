...
import { ArrowLeft, Plus, Minus, ShieldCheck, Clock, Boxes } from "lucide-react";
...
export const Route = createFileRoute("/shop/$productId")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
        meta: [{ title: "Commodity unavailable — AgriLink Kakuma" }, { name: "robots", content: "noindex" }],
      };
    }
    ...
    return {
      meta: [
        ...
        { property: "og:title", content: `${p.name} — AgriLink Kakuma B2B Catalog` },
      ...
      <section className="relative isolate overflow-hidden surface-forest">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          ...
          <p className="mt-3 text-primary-foreground/80">
            {product.category} · {product.grade} · {product.origin}
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.3fr_1fr] lg:px-8">
        <Reveal>
          <div className="space-y-8">
            <p className="text-lg text-muted-foreground">{product.blurb}</p>

            <div>
              <h2 className="text-lg font-extrabold uppercase">Specification</h2>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                { icon: ShieldCheck, label: "Grade", value: product.grade },
                { icon: Clock, label: "Lead time", value: product.lead },
                { icon: Boxes, label: "Minimum order", value: `${product.moq} ${product.unit}` },
              ].map((f) => (
                <div key={f.label} className="lift rounded-xl border border-border bg-card p-5">
        ...
        <Reveal delay={120}>
          <div className="sticky top-24 rounded-2xl border border-border bg-card p-7 shadow-sm">
            ...
            <div className="mt-6">
              <p className="text-xs font-semibold uppercase tracking-wider">Quantity ({product.unit})</p>
            ...
            <div className="mt-6 flex items-center justify-between border-t border-border pt-5">
              <span className="text-sm text-muted-foreground">Line estimate</span>
              <span className="font-display text-xl font-extrabold">
                ${(product.pricePerUnit * qty).toLocaleString()}
              </span>
            </div>
            ...
            <Button
              ...
              onClick={() => {
                add(product.id, qty);
                toast.success(`${qty} ${product.unit} of ${product.name} added to quote cart.`);
              }}
            >
              Add to quote cart
            </Button>
            <Button asChild variant="outline" className="mt-3 w-full">
              <Link to="/cart">View quote cart</Link>
            </Button>
          </div>
        </Reveal>
      </section>
